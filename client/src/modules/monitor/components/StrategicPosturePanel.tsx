...
const getPostureIcon = (level: string) => {
  switch (level) {
    case 'critical':
      return <ShieldAlert size={20} className="text-red-500 animate-pulse" />;
    case 'elevated':
...
const getPostureColor = (level: string) => {
  switch (level) {
    case 'critical':
      return 'text-red-400 bg-red-500/5 border-red-500/40 hover:bg-red-500/10 hover:border-red-500/60 shadow-[inset_0_0_15px_rgba(239,68,68,0.1)]';
    case 'elevated':
      return 'text-orange-400 bg-orange-500/5 border-orange-500/40 hover:bg-orange-500/10 hover:border-orange-500/60 shadow-[inset_0_0_15px_rgba(249,115,22,0.1)]';
    default:
    ...
    <div className="flex flex-col h-full space-y-4">
      <div className="flex items-center justify-between uppercase tracking-widest text-xs font-bold text-intel-text-light tech-panel-header pb-2">
        <div className="flex items-center gap-2">
          <Crosshair
            size={14}
            className="text-intel-accent drop-shadow-[0_0_8px_var(--color-intel-accent)]"
          />
              ...
              <div className="flex justify-between items-start mb-3 relative z-10">
                ...
                <span
                  className={clsx(
                    ...
                    theater.postureLevel === 'critical'
                      ? 'bg-red-500/20 border-red-500/50 text-red-300'
                      : theater.postureLevel === 'elevated'
              ...
              <div className="grid grid-cols-2 gap-2 relative z-10">
                <div className="flex items-center gap-2 bg-black/40 p-2 border border-intel-accent/10">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] text-intel-text uppercase tracking-wider">
                      Air Assets
                    </span>
                    <span className="font-mono text-sm font-bold text-intel-text-light">
                      {theater.activeFlights}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2 bg-black/40 p-2 border border-intel-accent/10">
              ...
              {theater.activeOperations.length > 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {theater.activeOperations.map((op) => (
                    <span
                      key={op}
                      className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded bg-white/10 text-white/70 border border-white/10"
                    >
                      {op.replace('_', ' ')}
                    </span>
                  ))}
