...
export function CyberMap() {
  ...
  const overlayPath =
    OVERLAY_ENDPOINTS[activeCategory] ?? '/radar/attacks/layer7/top/locations/origin';
        ...
        {popup && (
          ...
          >
            <PopupCard
              props={popup.props}
              color={circleColor}
              category={catDef?.name ?? activeCategory}
              onClose={() => setPopup(null)}
            />
          </Popup>
        )}
      ...
      <div className="absolute top-3 left-3 pointer-events-none z-10 tech-panel px-3 py-2 drop-shadow-[0_0_15px_rgba(0,229,255,0.2)]">
        <div className="flex items-center gap-2 text-xs font-mono font-bold tracking-[0.15em] text-intel-text-light uppercase">
          <span
            ...
            style={{ backgroundColor: circleColor, boxShadow: `0 0 8px ${circleColor}` }}
          />
          Cyber Threat Globe · {catDef?.name ?? 'Overview'}
          {geojson.features.length > 0 && (
            <span className="text-intel-text-light/40 ml-1 font-normal text-[10px]">
...
function PopupCard({
  ...
  const extra = Object.entries(props).filter(
    ([k]) => !KNOWN.has(k) && props[k] !== null && props[k] !== '',
  );
      ...
      <div className="px-3 py-2 flex flex-col gap-1.5">
        ...
        <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(value * 3, 100)}%`, backgroundColor: color }}
          />
        </div>
            ...
            <div className="border-t mt-1 pt-1.5" style={{ borderColor: `${color}22` }}>
              {extra.map(([k, v]) => (
                <div key={k} className="flex justify-between gap-2 py-0.5">
                  ...
                  <span className="text-white/70 text-right truncate max-w-[130px]">
                    {String(v)}
                  </span>
                </div>
              ))}
