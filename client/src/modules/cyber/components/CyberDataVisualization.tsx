...
// ─── Ranked Country ───────────────────────────────────────────────────────────
const RankedCountryRenderer: React.FC<{
    ...
    <div className="flex flex-col gap-1">
      {rows.map((row, i) => {
        ...
        return (
          <div
            key={i}
            className="flex items-center gap-2 bg-intel-bg/40 border border-white/5 px-3 py-1.5 rounded hover:bg-white/5 transition-colors"
          >
            <span className="text-intel-text-light/30 font-mono text-[10px] w-5 text-right shrink-0">
              {row.rank ?? i + 1}.
            </span>
            {alpha2 && <span className="text-sm leading-none shrink-0">{flagEmoji(alpha2)}</span>}
            <span className="text-intel-text-light font-mono text-xs flex-1 truncate pr-2">
              {name}
            </span>
            <Bar pct={raw} max={max} color={color} />
            <span
              className="font-mono text-xs font-bold tabular-nums w-16 text-right shrink-0"
              style={{ color }}
            >
              {raw.toFixed(2)}
              {unit ?? '%'}
...
// ─── Ranked ASN ───────────────────────────────────────────────────────────────
const RankedAsnRenderer: React.FC<{ rows: RankedAsnRow[]; color: string; unit?: string }> = ({
  rows,
  color,
  unit,
}) => {
...
// ─── Ranked Domain ────────────────────────────────────────────────────────────
const RankedDomainRenderer: React.FC<{ rows: RankedDomainRow[]; color: string }> = ({
  rows,
  color,
}) => (
  <div className="flex flex-col gap-1">
    {rows.map((row, i) => {
      ...
      return (
        <div
          key={i}
          className="flex items-center gap-3 bg-intel-bg/40 border border-white/5 px-3 py-1.5 rounded hover:bg-white/5 transition-colors"
        >
          <span
            className="font-mono text-xs font-bold w-10 text-right tabular-nums shrink-0"
            style={{ color }}
          >
            #{row.rank}
          </span>
          <span className="text-intel-text-light font-mono text-xs font-bold flex-1">
            {row.domain}
          </span>
          {cats && (
            <span className="text-intel-text-light/30 font-mono text-[10px] truncate max-w-[200px]">
              {cats}
            </span>
          )}
        </div>
...
// ─── Speed Table ──────────────────────────────────────────────────────────────
const SpeedTableRenderer: React.FC<{ rows: SpeedTableRow[]; color: string }> = ({
  rows,
  color,
}) => (
  <div>
    <div className="grid grid-cols-6 gap-2 px-3 py-1 mb-1">
      {['#', 'Country', 'Download', 'Upload', 'Latency', 'Jitter'].map((h) => (
        <span key={h} className="text-intel-text-light/30 font-mono text-[10px] uppercase">
          {h}
        </span>
      ))}
    </div>
    <div className="flex flex-col gap-1">
      {rows.map((row, i) => (
        <div
          key={i}
          className="grid grid-cols-6 gap-2 items-center bg-intel-bg/40 border border-white/5 px-3 py-1.5 rounded hover:bg-white/5 transition-colors"
        >
          ...
          <span className="font-mono text-xs font-bold tabular-nums" style={{ color }}>
            {parseFloat(row.bandwidthDownload).toFixed(0)}
            <span className="text-[9px] ml-0.5 opacity-60">Mbps</span>
          </span>
          <span className="font-mono text-xs text-intel-text-light/60 tabular-nums">
            {parseFloat(row.bandwidthUpload).toFixed(0)}
            <span className="text-[9px] ml-0.5 opacity-60">Mbps</span>
          </span>
          <span className="font-mono text-xs text-intel-text-light/60 tabular-nums">
            {parseFloat(row.latencyIdle).toFixed(0)}
            <span className="text-[9px] ml-0.5 opacity-60">ms</span>
          </span>
          <span className="font-mono text-xs text-intel-text-light/60 tabular-nums">
            {parseFloat(row.jitterIdle).toFixed(1)}
            <span className="text-[9px] ml-0.5 opacity-60">ms</span>
          </span>
        </div>
      ))}
  ...
  const groups = [
    {
      ...
      items: [
        ...
        {
          k: 'Invalid',
          v: stats.routes_invalid,
          pct: (stats.routes_invalid / stats.routes_total) * 100,
        },
        {
          k: 'Unknown',
          v: stats.routes_unknown,
          pct: (stats.routes_unknown / stats.routes_total) * 100,
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          k: 'IPv4',
          v: stats.distinct_prefixes_ipv4,
          pct: (stats.distinct_prefixes_ipv4 / stats.distinct_prefixes) * 100,
        },
        {
          k: 'IPv6',
          v: stats.distinct_prefixes_ipv6,
          pct: (stats.distinct_prefixes_ipv6 / stats.distinct_prefixes) * 100,
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          k: 'IPv4',
          v: stats.distinct_origins_ipv4,
          pct: (stats.distinct_origins_ipv4 / stats.distinct_origins) * 100,
        },
        {
          k: 'IPv6',
          v: stats.distinct_origins_ipv6,
          pct: (stats.distinct_origins_ipv6 / stats.distinct_origins) * 100,
        },
      ],
    },
  ];
    ...
    <div className="grid grid-cols-3 gap-4">
      {groups.map((g) => (
        <div key={g.label} className="bg-intel-bg/40 border border-white/5 rounded p-3">
          <p className="text-intel-text-light/40 font-mono text-[10px] uppercase tracking-widest mb-2">
            {g.label}
          </p>
    ...
    <div className="flex flex-col gap-2">
      {rows.map((a, i) => {
        ...
        return (
          <div
            key={i}
            className="bg-intel-bg/40 border rounded p-3 flex gap-3 relative overflow-hidden"
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span className="font-mono text-[9px] text-intel-text-light/40 uppercase">
                  {a.type}
                </span>
                {(a.visibleInDataSources ?? []).map((src: string) => (
                  <span
                    key={src}
                    className="font-mono text-[9px] text-intel-text-light/30 border border-white/10 px-1 rounded"
                  >
                    {src}
                  </span>
                ))}
                {!a.endDate && (
                  <span
                    className="font-mono text-[9px] px-1.5 py-0.5 rounded animate-pulse ml-auto shrink-0"
                    style={{ background: `${sc}22`, color: sc }}
                  >
                    ONGOING
                  </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-intel-text-light font-mono text-sm font-bold">
                  {name ?? 'Unknown'}
                </span>
                <span className="text-intel-text-light/30 font-mono text-[10px] ml-auto">
                  {duration}
                </span>
              </div>
            </div>
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="flex flex-col items-center gap-3">
          ...
          />
          <p
            className="font-mono text-[10px] tracking-[0.2em] uppercase animate-pulse"
            style={{ color }}
          >
            Acquiring Signal…
          </p>
  ...
  if (error) {
    return (
      <div className="bg-red-500/10 border border-red-500/30 rounded p-4 font-mono text-sm text-red-400">
        <div className="font-bold text-[10px] uppercase tracking-widest mb-1">
          Signal Intercepted — Error
        </div>
        {error.message}
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        />
        <span className="uppercase tracking-widest font-bold" style={{ color }}>
          {endpointDef.name}
        </span>
        <span className="text-intel-text-light/40 hidden md:inline">—</span>
        <span className="text-intel-text-light/50 hidden md:inline truncate">
          {endpointDef.description}
        </span>
        {rowCount !== null && (
          <span className="ml-auto shrink-0 font-mono text-intel-text-light/40">
            {rowCount} results
          </span>
        )}
        {lastUpdated && (
          <span className="text-intel-text-light/30 shrink-0">Updated {lastUpdated}</span>
        )}
        {dateRange && (
          <span className="text-intel-text-light/30 shrink-0">
            {new Date(dateRange.startTime).toLocaleDateString()} →{' '}
            {new Date(dateRange.endTime).toLocaleDateString()}
          </span>
        )}
      </div>
      ...
      >
        ...
        {endpointDef.renderer === 'bgp_stats' &&
          payload &&
          typeof payload === 'object' &&
          !Array.isArray(payload) && <BgpStatsRenderer stats={payload} color={color} />}
      </div>
    </div>
  );
};
