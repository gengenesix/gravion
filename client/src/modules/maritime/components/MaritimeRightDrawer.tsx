      ...
      <div className="flex items-start justify-between p-4 tech-panel-header">
        <div>
          ...
          <h2 className="text-2xl font-bold text-intel-text-light tracking-wider flex items-center drop-shadow-[0_0_8px_rgba(224,242,254,0.5)]">
            ...
            {isMoored && (
              <span className="ml-3 px-1.5 py-0.5 bg-[#f59e0b]/20 text-[#f59e0b] text-[10px] border border-[#f59e0b]/30 shadow-[0_0_10px_rgba(245,158,11,0.2)]">
                MOORED
              </span>
            )}
          </h2>
        ...
        <button
          onClick={onClose}
          className="text-intel-accent hover:text-intel-text-light hover:drop-shadow-[0_0_8px_var(--color-intel-accent)] transition-all font-mono mt-1"
        >
          [X]
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6">
        ...
        <div className="space-y-3 relative">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4">
            <div>
              <div className="text-[10px] text-intel-text uppercase tracking-widest mb-1 flex items-center">
                <Target size={10} className="mr-1.5" /> LATITUDE
              </div>
            ...
            <div>
              <div className="text-[10px] text-intel-text uppercase tracking-widest mb-1 flex items-center">
                <Target size={10} className="mr-1.5" /> LONGITUDE
              </div>
            ...
            <div>
              <div className="text-[10px] text-intel-text uppercase tracking-widest mb-1">
                SPEED OVER GROUND
              </div>
              <div className="text-lg font-medium text-intel-accent tabular-nums">
                {vessel.sog.toFixed(1)}{' '}
                <span className="text-xs text-intel-text font-normal">kn</span>
              </div>
            </div>
            <div>
              <div className="text-[10px] text-intel-text uppercase tracking-widest mb-1">
            ...
            {vessel.altitude !== undefined && (
              <div className="col-span-2 border-t border-white/5 pt-4 mt-2">
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-lg font-medium text-intel-accent tabular-nums">
                    {vessel.altitude} <span className="text-xs text-intel-text font-normal">m</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="space-y-4 pt-4 border-t border-white/10">
          ...
          <ul className="space-y-3 text-sm">
            <li className="flex flex-col">
              <span className="text-[10px] text-intel-text uppercase tracking-wider mb-0.5">
                NAVIGATIONAL STATUS
              </span>
              <span className="text-white font-medium">
                {getNavStatusString(vessel.navigationalStatus)}
              </span>
            </li>
            <li className="flex flex-col">
              <span className="text-[10px] text-intel-text uppercase tracking-wider mb-0.5">
              ...
              <span
                className={`text-white font-medium tabular-nums ${ageSeconds > 60 ? 'text-amber-400' : ''}`}
              >
                {ageSeconds}s ago
              </span>
            </li>
            ...
            {vessel.dimension && (vessel.dimension.a > 0 || vessel.dimension.b > 0) && (
              <li className="flex flex-col">
                ...
                <span className="text-white font-medium">
                  {vessel.dimension.a + vessel.dimension.b}m x{' '}
                  {vessel.dimension.c + vessel.dimension.d}m
                </span>
              </li>
            )}
            {vessel.history && (
              <li className="flex flex-col">
                ...
                <span className="text-white font-medium">
                  {vessel.history.length} position{vessel.history.length !== 1 ? 's' : ''}
                </span>
              </li>
            )}
          </ul>
