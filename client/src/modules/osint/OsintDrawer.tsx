...
export const OsintDrawer: React.FC = () => {
  ...
  const { data, isLoading, isError } = useOsintNews(
    currentRegionLat,
    currentRegionLon,
    selectedCategory,
    osintDrawerOpen,
    ...
    <div className="absolute left-0 top-0 bottom-8 w-96 tech-panel z-40 flex flex-col shadow-[15px_0_30px_rgba(0,0,0,0.8)] transition-transform transform translate-x-0 !border-l-0 !border-y-0 !border-r-intel-accent/30">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-sm no-scrollbar">
        <div className="text-intel-text/60 text-xs mb-4 uppercase tracking-widest">
          TARGETING SECTOR:{' '}
          {data?.news?.length ? data.news[0]?.source || 'INTERNATIONAL' : 'INTERNATIONAL'} (
          {currentRegionLat.toFixed(2)}, {currentRegionLon.toFixed(2)})
        </div>

        {isLoading && (
          <div className="text-intel-accent animate-pulse">&gt; INTERCEPTING COMMUNICATIONS...</div>
        )}

        {isError && (
          <div className="text-red-400">&gt; COMMS LINK SEVERED. UNABLE TO FETCH DATA.</div>
        )}

        {data?.intercepts?.map((intercept, idx) => (
          <div
            key={`intercept-${idx}`}
            className="border-l-2 border-red-500 pl-3 py-1 mb-4 bg-red-500/10"
          >
            <div className="text-red-400 font-bold text-xs mb-1">
              [CRITICAL INTERCEPT] {new Date(intercept.pubDate).toLocaleTimeString()}
            </div>
            <div className="text-intel-text-light">{intercept.title}</div>
          </div>
        ))}
        ...
        {data?.news?.map((item, idx) => (
          <div
            key={`news-${idx}`}
            className="border border-intel-accent/20 bg-black/40 p-3 hover:border-intel-accent/60 transition-colors shadow-[0_0_15px_rgba(0,229,255,0.02)] hover:shadow-[0_0_15px_rgba(0,229,255,0.1)]"
          >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-intel-text-light hover:text-intel-accent transition-colors font-semibold block mb-2 cursor-pointer"
            >
              {item.title}
            </a>
            <p className="text-intel-text/80 text-xs line-clamp-3">{item.snippet}</p>
          </div>
        ))}

        {data && data.news.length === 0 && !isLoading && (
          <div className="text-intel-text/60">&gt; NO SIGNALS COMING FROM THIS SECTOR.</div>
        )}
      </div>
