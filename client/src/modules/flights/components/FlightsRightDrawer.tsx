...
export const FlightsRightDrawer: React.FC<Props> = ({ flight, onClose }) => {
  ...
  return (
    <div className="absolute top-16 right-4 bottom-12 w-[340px] pointer-events-none z-10 flex flex-col">
      <Panel
        title={`intel: ${flight.callsign || flight.icao24}`}
        className="flex-1 overflow-y-auto overflow-x-hidden pt-4 pb-4"
      >
        <div
        ...
        <div className="space-y-6 mt-2 relative">
          ...
          <div className="w-full relative min-h-12 flex justify-center border-b border-intel-panel pb-4 mb-2">
            {photoLoading ? (
              ...
              <div className="w-full py-4 flex flex-col items-center justify-center border border-dashed border-intel-text-light/20 rounded-sm">
                ...
                <span className="text-[10px] text-intel-text-light/40 font-mono tracking-widest uppercase">
                  No Photo Available
                </span>
              </div>
            )}
          ...
          <div className="flex gap-2 border-b border-intel-panel pb-4 mb-2">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 border transition-all text-[11px] font-mono font-bold tracking-widest ${
                cameraTrackMode
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs font-mono border-b border-intel-panel pb-2">
            ...
            <div className="text-intel-accent text-right font-bold">
              {flight.callsign || 'UNKNOWN'}
            </div>
            <div className="text-intel-text uppercase">Country</div>
            <div
              className="text-intel-text-light text-right truncate"
              title={flight.originCountry || 'N/A'}
            >
          ...
          {(flight.registration || flight.model || flight.operator || flight.manufacturerName) && (
            <div>
              <div className="text-intel-text-light font-bold text-xs mb-2 uppercase border-l-2 border-intel-accent pl-2">
                Aircraft
              </div>
              <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] font-mono mb-6">
                {flight.registration && (
                  <>
                    ...
                    <div className="text-intel-accent font-bold text-right">
                      {flight.registration}
                    </div>
                  </>
                )}
                ...
                {(flight.manufacturerName || flight.model) && (
                  <>
                    ...
                    <div
                      className="text-intel-text-light text-right truncate"
                      title={`${flight.manufacturerName || ''} ${flight.model || ''}`.trim()}
                    >
                      {flight.manufacturerName ? `${flight.manufacturerName} ` : ''}
                      {flight.model || flight.typecode || 'Unknown'}
                    </div>
                  </>
                )}
                {flight.built && (
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] font-mono">
              ...
              <div className="text-intel-text-light text-right">
                {flight.velocity ? Math.round(flight.velocity * 1.94384) + ' kt' : 'n/a'}
              </div>
              ...
              <div className="text-intel-text-light text-right">
                {flight.geoAltitude ? Math.round(flight.geoAltitude * 3.28084) + ' ft' : 'n/a'}
              </div>
              ...
              <div className="text-intel-text-light text-right">
                {flight.verticalRate ? (
                  <span
                    className={flight.verticalRate > 0 ? 'text-intel-success' : 'text-intel-warn'}
                  >
              ...
              <div className="text-intel-text-light text-right">
                {flight.heading != null ? flight.heading.toFixed(1) + '°' : 'n/a'}
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] font-mono mb-6">
                {(flight.wd != null || flight.ws != null) && (
                  <>
                    ...
                    <div className="text-intel-text-light text-right">
                      {flight.wd != null ? `${flight.wd}° ` : ''}
                      {flight.ws != null ? `@ ${flight.ws} kt` : ''}
                    </div>
                  </>
                )}
                {flight.oat != null && (
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] font-mono">
              ...
              <div className="text-right">
                {flight.onGround ? (
                  <span className="text-intel-warn font-bold">GROUNDED</span>
                ) : (
                  <span className="text-intel-success font-bold">AIRBORNE</span>
                )}
              </div>
              ...
              <div className="text-intel-text-light text-right">
                {flight.spi ? 'Active' : 'False'}
              </div>

              <div className="text-intel-text">Pos. Source</div>
              <div className="text-intel-text-light text-right">
                {flight.positionSource === 0
                  ? 'ADS-B'
                  : flight.positionSource === 1
                    ? 'ASTERIX'
                    : flight.positionSource === 2
                      ? 'MLAT'
                      : 'FLARM'}
              </div>
              ...
              {flight.emergency && (
                <>
                  ...
                  <div className="text-intel-warn font-bold text-right uppercase animate-pulse">
                    {flight.emergency}
                  </div>
                </>
              )}
